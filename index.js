...
function get20s(data){
  ...
  for (let i = 0; i < data.length; i++) {
    if (data[i]['years'].split(' - ')[0] >= 1900) {
      if (data[i]['years'].split(' - ')[1] <= 2000) {
...
*/
function removeArtist(artistArr, desiredIndex) {
  artistArr.splice(desiredIndex, 1);
  return artistArr.length;
}

removeArtist(artists,0);

// normally I'd do this, but the problem asks for us to include console.log into the function
function removeArtist(artistArr, desiredIndex) {
    artistArr.splice(desiredIndex, 1);
    return artistArr.length;
  }
...
let newArtist = {
  ...
  bio: 'David was the worlds first superhero web devleoper. Kids looked up to him',
}
...
function lotsOfArt(artistsArray){
  ...
  for (let i = 0; i < artistsArray.length; i++) {
    artistsArray[i].paintings > 100 ? over100.push(artistsArray[i].name) : null;
  } return over100;
}
...
<div class = "name">
   <a href="${this.wikipedia}"> ${this.name}</a>
</div>
